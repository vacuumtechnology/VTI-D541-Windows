...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        // Convert point units from mm to m
        for (int i = 0; i < pointCloudPCL->points.size(); i++) {
            pointCloudPCL->points[i].x /= 1000;
            pointCloudPCL->points[i].y /= 1000;
            pointCloudPCL->points[i].z /= 1000;
        }

        pcl::io::savePCDFileBinary("../../pcd/downscaled.pcd", *pointCloudPCL);
